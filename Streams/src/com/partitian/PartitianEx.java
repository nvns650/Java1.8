...
public class PartitianEx {
	...
	public static void main(String[] args) {
		...
		Map<Boolean, List<Employee>> personsByIncome = persons.stream()
				.collect(Collectors.partitioningBy(s -> s.getIncome() > 5000));
		System.out.println(personsByIncome);

		Map<Boolean, String> personsByIncome2 = persons.stream().collect(
				Collectors.partitioningBy(s -> s.getDob().getYear() > 1975,
				Collectors.mapping(Employee::getName,Collectors.joining(","))));
		
		System.out.println(personsByIncome2);
